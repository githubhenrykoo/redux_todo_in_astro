...
const DEFAULT_URLS = ['https://www.youtube.com/watch?v=1WWweyBaWZk&ab_channel=GoingHeadlesswithJohn','https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps'];
...
const renderNestedPanel = (
  ...
  return (
    <iframe 
      ...
      title={`Panel iframe for ${component}`}
    />
  );
};

export default function urlResizablePanel({ 
  panelCount = DEFAULT_URLS.length,
  ...
  const generatePanels = (count: number) => {
    ...
    return Array(count).fill(0).map((_, index) => ({
      ...
      minSize: 10,
      maxSize: 90,
      defaultComponent: DEFAULT_URLS[index] || `Panel${index + 1}`,
          ...
          <React.Fragment key={config.id}>
            ...
            >
              {Array.isArray(config.defaultComponent) ? (
                ...
                <iframe 
                  src={config.defaultComponent} 
                  style={{ width: '100%', height: '100%', border: 'none' }}
                  title={`Panel iframe for ${config.defaultComponent}`}
                />
              )}
            </Panel>
            {index < panelCount - 1 && (
