// src/components/viewers/PNGViewer.jsx
import React, { useState, useEffect, useMemo } from 'react';

/**
 * A specialized viewer for PNG images
 */
export const PNGViewer = ({ content, contentType }) => {
  ...
  useEffect(() => {
    const processContent = () => {
      try {
        console.log("PNGViewer processing content:", typeof content, content instanceof Uint8Array);
        ...
        if (typeof content === 'object' && content !== null && 
          ...
          try {
            console.log("PNGViewer: Detected Node.js Buffer JSON format as object", content.data.length);
            ...
            setPngSource(`data:image/png;base64,${base64}`);
            return;
          } catch (e) {
            console.error("Error processing Buffer JSON format:", e);
            setError("Unable to display PNG: Error processing Buffer data");
            return;
          }
        }
        ...
        if (typeof content === 'string' && content.includes('"type":"Buffer"') && content.includes('"data":[')) {
          try {
            console.log("PNGViewer: Detected Node.js Buffer JSON format as string");
        ...
        if (content instanceof Uint8Array || content instanceof ArrayBuffer || 
          ...
          try {
            ...
            setError("Unable to display PNG: Error converting binary data");
            return;
          }
        }
        ...
        if (typeof content === 'object' && Array.isArray(content)) {
          try {
            ...
            setError("Unable to display PNG: Error processing array data");
            return;
          }
        }
        ...
        if (typeof content === 'string') {
          try {
            ...
            if (isBase64) {
              setPngSource(`data:image/png;base64,${content}`);
              return;
            } else {
            ...
            setError("Unable to display PNG: Invalid format");
            return;
          }
        }
        
        console.error("Unsupported content format:", typeof content, content);
        setError("Unable to display PNG: Unsupported format");
      } catch (err) {
        console.error("PNGViewer processing error:", err);
        ...
        setError(`Unable to display PNG: ${err.message || 'Unknown error'}`);
      }
    };
    ...
    return (
      <div className="flex flex-col h-full p-4 bg-gray-50 overflow-auto">
        <div className="text-amber-600 text-center mb-4">
          ...
          <p className="font-medium">Content has image/png type but appears to be text</p>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 bg-gray-50">
        <div className="text-red-500 text-center">
          ...
          <p className="text-sm text-gray-600 mt-2">File type: PNG</p>
          {size && <p className="text-sm text-gray-600">Size: {formatFileSize(size)}</p>}
        </div>
      </div>
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 bg-gray-50">
        <div className="text-gray-500 text-center">
          <div className="text-5xl mb-4">🖼️</div>
          <p className="font-medium">Unable to display PNG</p>
          <p className="text-sm text-gray-600 mt-2">
            The provided content could not be converted to a viewable PNG.
          </p>
        </div>
      </div>
    );
      ...
      <div className="relative max-w-full max-h-full overflow-auto">
        <img
          ...
          onError={(e) => {
            ...
            setIsLoading(false);
            setError("Failed to load PNG: Image data may be corrupted");
          }}
        />
      </div>
