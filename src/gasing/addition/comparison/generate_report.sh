#!/bin/bash
# Script to run benchmarks and generate the comparison report

echo "Starting Gasing Addition Algorithm Comparison Report Generation..."
...
pip install matplotlib numpy tabulate > /dev/null

# Check if LaTeX is installed
if ! command -v pdflatex &> /dev/null
then
    echo "WARNING: pdflatex not found. Report will be generated in LaTeX format only, not PDF."
    echo "To generate PDF, please install LaTeX (e.g., MacTeX or TexLive)."
fi

# Create a modified version of profsgasingaddition.py that returns values instead of printing
echo "Preparing modified version of professor's implementation..."
cat > profsgasingaddition_modified.py << 'EOF'
#!/usr/bin/env python3
"""
...
python3 benchmark_addition.py

# Check if the PDF was generated
if [ -f "benchmark_report.pdf" ]; then
    echo "Success! Report generated as benchmark_report.pdf"
else
    echo "LaTeX report generated as benchmark_report.tex"
    if command -v pdflatex &> /dev/null; then
        echo "Error generating PDF. Check for LaTeX errors in benchmark_report.log"
    fi
fi
